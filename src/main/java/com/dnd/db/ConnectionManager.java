...
public class ConnectionManager {
        ...
        if (dataSource == null) {
            synchronized (ConnectionManager.class) {
                if (dataSource == null) {
                    ...
                    log.info("HikariCP connection pool для D&D инициализирован");
                }
            }
        }
        return dataSource;
    ...
    public static Connection getConnection() throws SQLException {
        ...
        // Автоматически переключаем контекст соединения на схему dnd_game
        try (Statement statement = connection.createStatement()) {
            statement.execute("SET search_path TO dnd_game");
